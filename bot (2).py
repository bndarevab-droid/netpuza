...
roulette_message_count = 0   # счётчик сообщений с начала ивента roulette
...
def resolve_command(text: str):
    """Возвращает каноническое имя команды, если текст начинается с
    одного из префиксов и (точно или с опечаткой) похож на алиас."""
    if not text:
        return None
    ...
    # Допускаем небольшие опечатки в команде (только если префикс есть)
...
async def start_command(message: types.Message):
    ...
    # Обычные пользователи вместо админ-панели видят свои победы
...
# --- Приветствие при добавлении бота в определённую группу ---
@dp.my_chat_member(F.chat.id == WELCOME_GROUP_ID)
async def bot_added_to_welcome_group(event: ChatMemberUpdated):
...
async def cmd_mute(message: types.Message):
    ...
    # Команда "тише" - административная, доступна только админам чата
    ...
    # Нельзя применять административные команды к другим админам
...
async def group_message_handler(message: types.Message):
    ...
    if active_event == "roulette":
        ...
        # Первые 5 сообщений с начала ивента шанса не имеют
